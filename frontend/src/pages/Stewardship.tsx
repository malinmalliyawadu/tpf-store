...
import { useProductRecords } from "../hooks/useProductRecords";
...
export const Stewardship = () => {
    ...
    try {
      ...
      if (snapshot.exists()) {
        ...
        const matchedRecord = productRecords.find(
          (x) => x.serialNumber === serialNumberQuery
        );
        if (matchedRecord) {
  ...
  return (
    <Main>
      <Heading>Stewardship</Heading>
      <p className="mb-14">
        We take the stewardship of our products very seriously and aim to
        support users of our products throughout and at the end of their product
        life cycles. All of the products we sell come with an individual serial
        number (like shown in the picture on the left) which can be entered
        below to access the full manufacturing details and product
        specifications. In providing all the information about where your
        product came from, who made it, and when it was made, we hope to
        encourage users to form a special bond with their product: to care for
        and cherish it for a long time. We craft our products to last for many
        years, but if yours does ever meet an unfortunate end, simply package it
        up and we’ll pay to ship it back to us for recycling (again!) and even
        give you discount on a new one.
      </p>
      ...
      <div className="mb-14">
        ...
        <form className="flex items-center mb-14" onSubmit={(e) => doSearch(e)}>
